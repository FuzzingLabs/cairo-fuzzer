...
use crate::mutator::magic_values;
use alloc::vec::Vec;
use felt::Felt252;
use magic_values::MAGIC_VALUES;
use crate::mutator::mutator_felt252; // Add the missing import statement
use super::{mutator::Mutator, rng::Rng, types::Type};

pub struct FeltMutator {
    pub mutator: mutator_felt252::BasicMutator,
}

impl FeltMutator{
    pub fn new(seed: u64, max_input_size: usize) -> Self { // Remove the unnecessary semicolon after the field declaration
        let mutator = crate::mutator::mutator_felt252::BasicMutator::new()
            .seed(seed)
...
impl From<Type> for Felt252 {
    fn from(value: Type) -> Self {
        match value {
            ...
            Type::U128(_) => todo!(),
            Type::Bool(_) => todo!(),
            Type::Vector(t, _) => Felt252::from(b'\0'),
            
        }
    }
}

impl Mutator for FeltMutator {
    fn mutate(&mut self, input: &Vec<Type>, mutations: usize) -> Vec<Type> {
            let converted_input: Vec<Felt252> = input.iter().map(|item| {
        // Assuming convert_to_felt252 is a function that converts Type to Felt252
        Felt252::from(item.to_owned())
    }).collect();
        self.mutator.input = converted_input; // Dereference the input reference
        self.mutator.mutate(mutations, &mut mutator_felt252::EmptyDatabase);
        let converted_input_res: Vec<Type> = self.mutator.input.iter().map(|item| {
        // Assuming convert_to_felt252 is a function that converts Type to Felt252
        Type::Felt252(item.to_owned())
    }).collect();
    converted_input_res.clone()
    }
}
...
impl BasicMutator{
    ...
    pub fn mutate<T: InputDatabase>(&mut self, mutations: usize, inputs: &T) {
        ...
        const STRATEGIES: &[fn(&mut BasicMutator)] = &[
            //BasicMutator::shrink,
            //BasicMutator::expand,
            BasicMutator::inc_byte,
            BasicMutator::dec_byte,
            BasicMutator::neg_byte,
            BasicMutator::add_sub,
            //BasicMutator::set,
            BasicMutator::swap,
            BasicMutator::copy,
            BasicMutator::inter_splice,
            //BasicMutator::insert_rand,
            BasicMutator::overwrite_rand,
            //BasicMutator::byte_repeat_overwrite,
            //BasicMutator::byte_repeat_insert,
            BasicMutator::magic_overwrite,
            //BasicMutator::magic_insert,
            BasicMutator::random_overwrite,
            //BasicMutator::random_insert,
            BasicMutator::splice_overwrite,
            //BasicMutator::splice_insert,
        ];
        ...
        for _ in 0..mutations {
            ...
            let splice_overwrite =
                core::ptr::eq(strat as *const (), BasicMutator::splice_overwrite as *const ());
            let splice_insert =
                core::ptr::eq(strat as *const (), BasicMutator::splice_insert as *const ());
    ...
    fn expand(&mut self) {
        ...
        let iter = if self.printable {
            core::iter::repeat(Felt252::from(b' ')).take(self.rng.rand_exp(1, max_expand))
        } else {
            core::iter::repeat(Felt252::from(b'\0')).take(self.rng.rand_exp(1, max_expand))
        };
    ...
    fn insert_inplace(&mut self, source: usize, len: usize, dest: usize) {
        ...
        let rep = core::iter::repeat(Felt252::from(b'\0')).take(len);
    ...
    fn insert_rand(&mut self) {
        ...
        let bytes = if self.printable {
            [
                Felt252::from(self.rng.rand(0, 94) + 32),
                Felt252::from(self.rng.rand(0, 94) + 32),
            ]
        } else {
            [
                Felt252::from(self.rng.rand(0, 255)),
                Felt252::from(self.rng.rand(0, 255)),
            ]
        };
        ...
        let offset = self.rand_offset_int(true);
        self.insert(offset, bytes[0].clone());
        // Pick a random offset
        let offset = self.rand_offset_int(true);
        self.insert(offset, bytes[1].clone());
    }
    ...
    fn random_overwrite(&mut self) {
        ...
        if self.printable {
            self.input[offset..offset + amount]
                .iter_mut()
                .for_each(|x| {
                    *x = Felt252::from(rng.rand(0, 94) + 32);
                });
        } else {
            self.input[offset..offset + amount]
                .iter_mut()
                .for_each(|x| *x = Felt252::from(rng.rand(0, 255)));
        }
    }
    ...
    fn random_insert(&mut self) {
        ...
        if self.printable {
            self.input.splice(
                offset..offset,
                (0..amount).map(|_| Felt252::from(rng.rand(0, 94) + 32)),
            );
        } else {
            self.input.splice(
                offset..offset,
    ...
    /*     byte_corruptor!(bit, |obj: &mut Self, x: Felt252| -> Felt252 {
        x ^ Felt252::from(1u8 << obj.rng.rand(0, 7))
    }); */
